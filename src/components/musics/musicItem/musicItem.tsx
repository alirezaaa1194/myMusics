...
function MusicItemComp({ music }: { music: PrismaType.Music }) {
  ...
  return (
    <div className={`flex items-center justify-between gap-10 ${globalContext?.options?.currentMusicId === music.id ? "bg-surface/60" : "bg-surface"} rounded-lg px-3 border border-border`}>
      <div className="flex items-center gap-4 w-full cursor-pointer py-1" onClick={playMusicHandler}>
        ...
        <div className="flex flex-col gap-[2px] lg:gap-1">
          <span className="text-white text-[15px] lg:text-sm font-[500] line-clamp-1">{music.title}</span>
          <span className="text-secondary2 text-[14px] lg:text-xs line-clamp-1">{music.singerName}</span>
        </div>
      </div>
      <div className="flex items-center gap-10 shrink-0 py-1">
