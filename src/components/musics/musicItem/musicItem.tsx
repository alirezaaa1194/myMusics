...
function MusicItemComp({ music }: { music: PrismaType.Music }) {
  ...
  return (
    <div className={`flex items-center justify-between gap-10 ${globalContext?.options?.currentMusicId === music.id ? "bg-surface/70" : "bg-surface"} rounded-lg px-3 py-1 border border-border`}>
      <div className="flex items-center gap-4 w-full cursor-pointer" onClick={playMusicHandler}>
        ...
        <div className="flex flex-col gap-1">
          <span className="text-white text-sm font-[500]">{music.title}</span>
          <span className="text-secondary2 text-xs">{music.singerName}</span>
        </div>
      </div>
