...
function PlayBtnComp({ className }: { className?: string }) {
  const globalContext = use(GlobalContext);

  const audio = globalContext?.audio.current;
  ...
  return (
    <Button
      className={`${className ? className : "w-16 h-16 rounded-full cursor-pointer"}`}
      onClick={() => {
        if (globalContext) {
          if (globalContext?.play) {
            globalContext.setPlay(false);
