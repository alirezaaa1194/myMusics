...
export const uploadFormSchema = z.discriminatedUnion("musicType", [
  z.object({
    ...
    musicFile: z.any().optional(), // وجود داره ولی لازم نیست
  }),
  z.object({
    ...
    musicLink: z.string().optional(), // وجود داره ولی لازم نیست
  }),
]);

export const loginFormSchema = z.object({
  email: z.email("Please enter correct email!"),
  password: z.string().min(8, "Password must be 8 char"),
});

export const signupFormSchema = z.object({
  username: z.string().min(4, "Username must be 4 char"),
  email: z.email("Please enter correct email!"),
  password: z.string().min(8, "Password must be 8 char"),
});

export const folderFormSchema = z.object({
  title: z.string().min(1, "Please enter folder name"),
});
